...
/**********************************************/
* INRAW1 fileref is defined in autoexec.sas (bundled crosswalk sample).;
LIBNAME  IN1     (WORK);                                          * Input discharge data (built in autoexec).;
LIBNAME  OUT1    (WORK);                                          * Output data (WORK for this run).;
...
* Specify the maximum number of diagnosis codes
  on any record in the input file. ;                        %LET NUMDX =5;        *<=== USER MUST MODIFY;
...
%macro main;
   %count_ccsr;
   proc sort data=IN1.&CORE(%if %upcase(&OBS) ne MAX %then obs=&OBS; keep=&RECID &NDXVAR &DXPREFIX.1-&DXPREFIX.&NUMDX %if %upcase(&dbtype) = IO and &IOVAR ne %then &IOVAR;) out=&CORE.Skinny; by &RECID;  run;
